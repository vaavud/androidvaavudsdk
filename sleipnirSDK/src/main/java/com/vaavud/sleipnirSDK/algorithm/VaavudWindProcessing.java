...
public class VaavudWindProcessing {
    ...
    private int[] tickEdgeAngle = new int[TEETH_PR_REV];
    ; // add one point in ether end
...
//	            Log.d("VaavudWindProcessing","Previous Tick Time:" + previousTickTime);
        ...
        // update results
        Date time = new Date();
        if (time.getTime() > previousTickTime + UPDATE_INTERVAL && tickCounterSinceStart > TEETH_PR_REV) {
            updateUI();
